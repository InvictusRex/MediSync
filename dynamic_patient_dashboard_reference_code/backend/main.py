...
from database import SessionLocal, get_formatted_time
...
@app.post("/login")
def login_user(user: schemas.UserLogin, db: Session = Depends(get_db)) -> JSONResponse:
    try:
...
@app.get("/patient-dashboard/{user_id}")
def get_patient_dashboard(
    ...
    try:
        ...
        if user.role != "patient":
            raise HTTPException(status_code=403, detail="Access denied. This endpoint is for patients only.")

        appointments = crud.get_user_appointments(db, user_id, limit=10)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch patient dashboard data: {str(e)}"
        )
